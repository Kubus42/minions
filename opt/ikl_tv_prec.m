function [x, varargout] = ikl_tv_prec(f,op,lambda,L,varargin)
...
while ( it <= niter )
    Ax_hat = A(x_hat);
    y      = x_hat - tau * K .* At(1 - (f./ Ax_hat).*(Ax_hat>0)); 
    tol   = 1 / ((1 + 2 * theta)*(it+1)^(1+alpha));
    inv_K = 1./K;
    [x,h]     = prox_tv_pos(y,lambda*tau,'K',inv_K,'norm_type',norm_type,'tol',tol);
