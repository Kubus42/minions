function [x, x_avg ,varargout] = ipd_l2_tv_smooth(f,op,lambda,varargin)
...
% Deblurring step sizes
tau   = sqrt(4 + 4*L^2/epsilon) / (2*epsilon + 2*L^2); 
sigma = sqrt(4 + 4*L^2/epsilon) / (2 + 2*L^2/epsilon); 
theta = ( (2*L^2 + 2 * epsilon) / epsilon - sqrt(4 + 4*L^2/epsilon) ) / ((2*L^2)/epsilon);
...
while ( it <= niter && PDres > pdtol )
    ...
    % Primal update
    if (strcmp(start,'warm') ~= 1)
        [x,h]  = prox_tv((1-tau*epsilon) * x-tau*Aty,tau*lambda,'tol',tol,'err','gap');
    else
        [x,h,y0]  = prox_tv((1-tau*epsilon) * x-tau*Aty,tau*lambda,'tol',tol,'err','gap','y0',y0);
%         [x,h,y0]  = prox_tv(x-tau*Aty,tau*lambda,'tol',1e-10,'err','gap','y0',y0,'niter',100);
    end
        
    % Ergodic average 
    x_avg = x_avg + 1/theta^(it-1) * x;
    y_avg = y_avg + 1/theta^(it-1) * y;
    tau_avg = tau_avg + 1/theta^(it-1);
